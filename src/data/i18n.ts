...
export const i18n = {
  ...
  zh: {
    ...
    playbook: {
      ...
      body: "四条规则。每一条都是我熬过的一个周末换来的。可以直接抄走 —— 不挑技术栈、不挑 Agent。",
      steps: [
        {
          icon: "sparkles",
          title: "逼它先说出假设",
          antiPattern: "沉默的猜测",
          body: "模型会自己挑一种解释，然后闷头跑，从不告诉你。任何不简单的任务，强制它先一句话回答：「你对输入、范围和副作用做了哪些假设？」",
          example: "我的「不是这意思，重写」次数减少了约 40%。它花 10 秒复述任务，省下我 20 分钟撤回。",
        },
        {
          icon: "layers",
          title: "验证效果，不要验证命令",
          antiPattern: "退出码 0 的剧场",
          body: "`pnpm build` 退出 0 不代表页面能打开；`git push` 成功不代表线上更新了。每一个动作都要有一个独立的检查，去看真实的副作用。",
          example: "我给每个脚本都配一个 one-liner：build → grep dist 里的预期字符串；deploy → curl 线上 URL 找新标记。grep 不到，就不算绿。",
        },
        {
          icon: "terminal",
          title: "让 diff 比对话还短",
          antiPattern: "200 行漂移",
          body: "Agent 喜欢顺手「优化」旁边的代码。于是一个 1 行的 bug 修复，变成一个 17 文件的 PR，没人 review 得动。每一行改动都要能追溯到原始需求，否则回滚。",
          example: "我 CLAUDE.md 里的硬规则：每一步之后 `git diff --stat`。改动的文件数 > 提到的文件数，立刻停手，剪掉多余的。",
        },
        {
          icon: "layers",
          title: "把上下文当内存用，不是硬盘",
          antiPattern: "一个超长会话",
          body: "长会话会腐烂：模型忘掉早期决定、偏离原始目标、把旧项目的模式带到新项目里。把决定写进文件，探索性任务交给 subagent，新会话开得狠一点。",
          example: "经验法则：一个任务 = 一个会话。需要超过 20 次工具调用的，其实是 3 个任务伪装成 1 个。",
        },
      ],
